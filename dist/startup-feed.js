...
// Public, historical founder introductions. These are examples, not event attendees.
const startups=[
  {id:'doordash',video:'Rzlr2tNSl0U',name:'DoorDash',initial:'D',color:'#e6533d',founder:'Tony, Stanley, Andy & Evan',role:'Founding team',batch:'YC S13',category:'Local commerce',title:'Helping local businesses deliver.',description:'Connecting local restaurants, customers, and drivers through on-demand delivery.',tags:['Marketplace','Logistics'],source:'https://www.ycombinator.com/blog/doordash-from-application-to-ipo/'},
  {id:'zenefits',video:'-S83fysRwn4',name:'Zenefits',initial:'Z',color:'#df7237',founder:'Parker Conrad',role:'Founder',batch:'YC W13',category:'HR software',title:'Benefits without the busywork.',description:'Making it easier for small businesses to manage employee insurance and benefits.',tags:['B2B','HR tech'],source:'https://www.ycombinator.com/video'},
  {id:'campusjob',video:'4nxrkPtR348',name:'Campus Job',initial:'C',color:'#577adb',founder:'The Campus Job team',role:'Co-founders',batch:'YC W15',category:'Recruiting',title:'Connecting students with opportunity.',description:'A marketplace bringing students and employers together.',tags:['Marketplace','Recruiting'],source:'https://www.ycombinator.com/video'}
];
...
let observer,playbackTimer,toastTimer,resumeAfterDialog=false,renderGeneration=0;
let players=new Map(),readyPlayers=new Set(),manualPauses=new Set();
...
function command(id,method,...args){if(readyPlayers.has(id)){try{return players.get(id)?.[method]?.(...args)}catch{}}}
function pauseAll(){players.forEach((_,id)=>command(id,'pauseVideo'))}
function canPlay(id){return currentTab==='feed'&&id===activeId&&!document.hidden&&!modal.open&&!manualPauses.has(id)}
function play(id){if(canPlay(id)){command(id,muted?'mute':'unMute');command(id,'playVideo')}}
function teardown(){observer?.disconnect();clearInterval(playbackTimer);players.forEach(p=>{try{p.destroy()}catch{}});players.clear();readyPlayers.clear()}
function render(){
...
async function initializePlayers(generation){
  try{await loadYouTube()}catch{if(generation===renderGeneration)startups.forEach(s=>showPlayerError(s.id));return}
  if(generation!==renderGeneration||currentTab!=='feed')return;
  startups.forEach(s=>{
    const player=new YT.Player(`player-${s.id}`,{host:'https://www.youtube-nocookie.com',width:'100%',height:'100%',videoId:s.video,
      playerVars:{playsinline:1,rel:0,origin:location.origin,controls:1,autoplay:0},
      events:{onReady(event){if(generation!==renderGeneration)return;readyPlayers.add(s.id);event.target.mute();const iframe=event.target.getIframe();iframe.title=`${s.name} founder pitch`;iframe.setAttribute('referrerpolicy','strict-origin-when-cross-origin');document.querySelector(`[data-startup="${s.id}"] .player-loading`).hidden=true;document.querySelector(`[data-startup="${s.id}"] .player-error`).hidden=true;if(!reducedMotion)play(s.id)},
        onStateChange(event){if(generation!==renderGeneration)return;if(event.data===1){if(currentTab!=='feed'||activeId!==s.id||document.hidden||modal.open){command(s.id,'pauseVideo');return}manualPauses.delete(s.id)}if(event.data===2&&canPlay(s.id))manualPauses.add(s.id);if(event.data===0&&canPlay(s.id)){command(s.id,'seekTo',0);play(s.id)}const button=document.querySelector(`[data-startup="${s.id}"] [data-action="play"]`);if(button){button.innerHTML=icon(event.data===1?'pause':'play');button.setAttribute('aria-label',`${event.data===1?'Pause':'Play'} ${s.name} pitch`)}},
        onError(){if(generation===renderGeneration)showPlayerError(s.id)}}});players.set(s.id,player);
    setTimeout(()=>{if(generation===renderGeneration&&!readyPlayers.has(s.id))showPlayerError(s.id)},15000);
  });
}
function initFeed(){
  ...
  observer=new IntersectionObserver(entries=>{for(const entry of entries){const id=entry.target.dataset.startup;if(entry.isIntersecting&&entry.intersectionRatio>.65){activeId=id;players.forEach((_,other)=>{if(other!==id)command(other,'pauseVideo')});if(!reducedMotion)play(id);updateArrows()}else command(id,'pauseVideo')}},{root:viewport,threshold:[0,.65]});
  ...
  initializePlayers(renderGeneration);updateSound();updateArrows();
  playbackTimer=setInterval(()=>{const duration=command(activeId,'getDuration')||0,current=command(activeId,'getCurrentTime')||0;const bar=document.querySelector(`[data-startup="${activeId}"] .pitch-progress span`);if(bar)bar.style.width=duration?`${current/duration*100}%`:'0%'},300);
}
...
function moveVideo(step){const next=startups[startups.findIndex(s=>s.id===activeId)+step];if(next){const viewport=document.querySelector('.pitch-viewport');viewport.scrollTo({top:document.querySelector(`[data-startup="${next.id}"]`).offsetTop,behavior:reducedMotion?'instant':'smooth'})}}
function updateSound(){players.forEach((_,id)=>command(id,muted?'mute':'unMute'));document.querySelectorAll('.sound-control').forEach(b=>{b.innerHTML=icon(muted?'mute':'sound');b.setAttribute('aria-label',muted?'Unmute pitch':'Mute pitch')})}
...
function showModal(title,body,kind=''){
  if(!modal.open){resumeAfterDialog=command(activeId,'getPlayerState')===1;pauseAll()}
...
function showStartup(s){showModal(s.name,`<div class="startup-profile-head">${badge(s)}<div><strong>${s.category}</strong><p>${s.batch} · Historical pitch</p></div></div><h3>${s.title}</h3><p>${s.description}</p><div class="startup-tags">${s.tags.map(t=>`<span>${t}</span>`).join('')}</div><div class="profile-founder">${icon('people')}<div><strong>${s.founder}</strong><span>${s.role}</span></div></div><button class="primary-button full" data-action="message" data-id="${s.id}">${icon('message')} Message startup</button><a class="credit-row" href="${s.source}" target="_blank" rel="noopener noreferrer">View original pitch source ↗</a><p class="demo-note">Example profile using an archived YC pitch. This company is not connected to the demo.</p>`,'startup-sheet')}
...
document.addEventListener('click',async event=>{
  ...
  if(action==='play'){if(command(id,'getPlayerState')===1){manualPauses.add(id);command(id,'pauseVideo')}else{manualPauses.delete(id);play(id)}}
  ...
  if(action==='retry'){const region=el.closest('.pitch-player-region');region.querySelector('.player-error').hidden=true;region.querySelector('.player-loading').hidden=false;render()}
  ...
  if(action==='credits')showModal('About these pitches',`<p>Public, historical YC application videos, embedded from YouTube. Example startup profiles are not connected to the companies. Messages are saved locally and never delivered.</p>${startups.map(s=>`<a class="credit-row" href="${s.source}" target="_blank" rel="noopener noreferrer">${s.name} · ${s.batch} ↗</a>`).join('')}`);
});
